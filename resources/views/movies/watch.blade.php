...
<style type="text/css">
    .vpl-settings-menu .vpl-quality-menu .vpl-menu-item.vpl-btn-reset{display: block;}
    /*.vpl-settings-menu .vpl-quality-menu .vpl-menu-item.vpl-btn-reset.vpl-menu-active{display: none;}*/
</style>
...
<?php
    ...
    $qualities = [];
    if(!is_null($movie->video_url_1080p) && $movie->video_url_1080p !== ''){
        $qualities[] = 'video_url_1080p';
    }
    if(!is_null($movie->video_url_720p) && $movie->video_url_720p !== ''){
        $qualities[] = 'video_url_720p';
    }
    if(!is_null($movie->video_url_480p) && $movie->video_url_480p !== ''){
        $qualities[] = 'video_url_480p';
    }
    if(!is_null($movie->video_url) && $movie->video_url !== ''){
        $qualities[] = 'video_url';
    }
...
<script type="text/javascript">
    ...
    document.addEventListener("DOMContentLoaded", function(event) { 
        QualitySelectorVideoLoad(playbacktime,video_url);
    });
    $(document).on('click','.vpl-btn-reset',function(){
        if(!$(this).hasClass('qualityItem')){
            return false;
        }
    });

    $(document).on('click', '.qualityItem', function() {
        var dataValue = $(this).data('value');
        var movie_id = "{{$movie->id}}";
        var getCurrentTime=parseInt(player.getCurrentTime(),10);
        $.ajax({
            type: 'POST',
            url: "{{ route('videoquality') }}",
            data: { dataValue: dataValue, _token:csrfToken,movie_id: movie_id},
            success: function (result) {
                var video_url = result['movie_url'];
                QualitySelectorVideoLoad(getCurrentTime,video_url,dataValue);
            }
        });
    });

    function QualitySelectorVideoLoad(playbacktime,video_url,selectedQuality){
        //let playbacktime='{{ empty($usermovie)?0:$usermovie->watch_time }}';
        ...
        .then(content => {            
            ...
            var quality = '<div role="menu" class="vpl-quality-menu">';
            @foreach($qualities as $quality)
            var qualityType = "{{$quality}}";
            if(qualityType == 'video_url'){
                qualityType_url = "auto";
            }else{
                qualityType = qualityType.split("_");
            }
            if(selectedQuality == '{{$quality}}'){
                if(qualityType == 'video_url'){
                    quality += '<button type="button" class="vpl-menu-item vpl-btn-reset vpl-menu-active qualityItem" role="menuitemradio" aria-checked="false" tabindex="0" data-value="{{$quality}}">'+qualityType_url+'</button>';
                }else{
                    quality += '<button type="button" class="vpl-menu-item vpl-btn-reset vpl-menu-active qualityItem" role="menuitemradio" aria-checked="false" tabindex="0" data-value="{{$quality}}">'+qualityType[2]+'</button>';
                }

            }else{
                if(qualityType == 'video_url'){
                    quality += '<button type="button" class="vpl-menu-item vpl-btn-reset qualityItem" role="menuitemradio" aria-checked="false" tabindex="0" data-value="{{$quality}}">'+qualityType_url+'</button>';
                }
                else{
                    quality += '<button type="button" class="vpl-menu-item vpl-btn-reset qualityItem" role="menuitemradio" aria-checked="false" tabindex="0" data-value="{{$quality}}">'+qualityType[2]+'</button>';
                }
            }
            @endforeach
            quality += '</div>';
            ...
            $('.vpl-quality-menu').replaceWith(quality);
